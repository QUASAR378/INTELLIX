...
const MiniGridSim = ({ initialConfig, countyName, autoStart = false, onSimulationComplete }) => {
  ...
  useEffect(() => {
    let interval;
    if (isRealTime && simulation) {
      console.log(`🔴 Starting red line animation - isRealTime: ${isRealTime}`);
          ...
          <div className="flex items-center space-x-4">
            {autoStart ? (
              ...
              <>
                ...
                >
                  {isRealTime ? '⏹️ Stop Live' : '▶️ Live Mode'}
                </button>
                <button
          ...
          <div className="mb-3 p-2 bg-red-100 border border-red-300 rounded-lg">
            <div className="flex items-center justify-between text-sm">
              <div className="flex items-center space-x-2">
                <div className="w-2 h-2 bg-red-500 rounded-full animate-pulse"></div>
                <span className="text-red-700 font-medium">🔴 RED LINE ANIMATION ACTIVE</span>
              </div>
          ...
          <div className="bg-gradient-to-br from-yellow-50 to-amber-50 rounded-lg p-4 border border-yellow-200">
            <div className="text-center">
              <div className="text-2xl font-bold text-yellow-600">
                {config.households_served}
              </div>
              <div className="text-sm text-yellow-700">Households Served</div>
            </div>
          </div>
        </div>
      )}
